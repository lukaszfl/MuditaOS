                                              ...
                                              transfer_id xfrId) -> void
{
    readTransfers.insert(
        std::make_pair(xfrId, std::make_unique<FileReadContext>(file, fileSize, 0, FileOperations::ChunkSize)));
}
                                               ...
                                               transfer_id xfrId) -> void
{
    writeTransfers.insert(
        std::make_pair(xfrId, std::make_unique<FileWriteContext>(file, fileSize, Crc32, 0, FileOperations::ChunkSize)));
}
...
auto FileOperations::getDataForReceiveID(transfer_id rxID, std::uint32_t chunkNo) -> std::string
{
    ...
    if (fileCtx->reachedEOF()) {
        ...
        LOG_DEBUG("Hash %s", fileCtx->fileHash().c_str());
    ...
    -> sys::ReturnCodes
{
    ...
    fileCtx->sendDataForFile(binaryData);
