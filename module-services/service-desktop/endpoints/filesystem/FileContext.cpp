...
FileContext::FileContext(
    std::filesystem::path path, std::size_t size, std::size_t offset, std::size_t chunkSize, std::string openMode)
    : path(path), size(size), offset(offset), chunkSize(chunkSize)
{
    ...
    fileDes = std::fopen(path.c_str(), openMode.c_str());
...
FileReadContext::FileReadContext(std::filesystem::path path,
    ...
    : FileContext(path, size, offset, chunkSize, "rb")
{}
...
FileWriteContext::FileWriteContext(
    std::filesystem::path path, std::size_t size, std::string crc32Digest, std::size_t offset, std::size_t chunkSize)
    : FileContext(path, size, offset, chunkSize, "wb"), crc32Digest(crc32Digest)
{}
...
auto FileReadContext::getFileHash() -> std::string
{
    ...
    while (dataSize) {
        ...
        auto dataRead = std::fread(buffer.data(), sizeof(int8_t), dataLeft, fileDes);
...
auto FileReadContext::getDataForFile() -> std::vector<std::uint8_t>
{
    LOG_DEBUG("Getting file data");

    std::fseek(fileDes, offset, SEEK_SET);
...
auto FileWriteContext::sendDataForFile(const std::vector<std::uint8_t> &data) -> void
{
    ...
    auto dataWritten = std::fwrite(reinterpret_cast<const char *>(data.data()), sizeof(int8_t), dataLeft, fileDes);
