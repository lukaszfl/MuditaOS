...
SettingsRecord DBServiceAPI::SettingsGet(sys::Service *serv)
{
    ...
    if (ret.first == sys::ReturnCodes::Success)
    {
        ...
        if (settingsResponse->retCode == true) { return settingsResponse->record; }
        else
        {
...
bool DBServiceAPI::SettingsUpdate(sys::Service *serv, const SettingsRecord &rec)
{
    ...
    auto ret = sys::Bus::SendUnicast(msg, ServiceDB::serviceName, serv, 5000);
    if (ret.first == sys::ReturnCodes::Success) { return true; }
    else
    {
        return false;
    }
...
bool DBServiceAPI::SMSAdd(sys::Service *serv, const SMSRecord &rec)
{
    ...
    if ((ret.first == sys::ReturnCodes::Success) && (smsResponse->retCode == true)) { return true; }
    else
    {
        return false;
    }
...
std::unique_ptr<std::vector<SMSRecord>> DBServiceAPI::SMSGetLimitOffset(sys::Service *serv, uint32_t offset, uint32_t limit)
{
    ...
    auto ret = sys::Bus::SendUnicast(msg, ServiceDB::serviceName, serv, 5000);
    DBSMSResponseMessage *smsResponse = reinterpret_cast<DBSMSResponseMessage *>(ret.second.get());
    if ((ret.first == sys::ReturnCodes::Success) && (smsResponse->retCode == true)) { return std::move(smsResponse->records); }
    else
    {
...
ThreadRecord DBServiceAPI::ThreadGet(sys::Service *serv, uint32_t id)
{
    ...
    if ((ret.first == sys::ReturnCodes::Success) && (threadResponse->retCode == true)) { return std::move((*threadResponse->records)[0]); }
    else
    {
        return ThreadRecord{};
    }
...
bool DBServiceAPI::ThreadRemove(sys::Service *serv, uint32_t id)
{
    ...
    auto ret = sys::Bus::SendUnicast(msg, ServiceDB::serviceName, serv, 5000);
    DBThreadResponseMessage *threadResponse = reinterpret_cast<DBThreadResponseMessage *>(ret.second.get());
    if ((ret.first == sys::ReturnCodes::Success) && (threadResponse->retCode == true)) { return true; }
    else
    {
        return false;
    }
...
std::unique_ptr<std::vector<ThreadRecord>> DBServiceAPI::ThreadGetLimitOffset(sys::Service *serv, uint32_t offset, uint32_t limit)
{
    ...
    auto ret = sys::Bus::SendUnicast(msg, ServiceDB::serviceName, serv, 5000);
    DBThreadResponseMessage *threadResponse = reinterpret_cast<DBThreadResponseMessage *>(ret.second.get());
    if ((ret.first == sys::ReturnCodes::Success) && (threadResponse->retCode == true)) { return std::move(threadResponse->records); }
    else
    {
...
std::unique_ptr<std::vector<ContactRecord>> DBServiceAPI::ContactGetByName(sys::Service *serv, UTF8 primaryName, UTF8 alternativeName)
{
    ...
    if ((ret.first == sys::ReturnCodes::Success) && (contactResponse->retCode == true)) { return std::move(contactResponse->records); }
    else
    {
...
bool DBServiceAPI::ContactAdd(sys::Service *serv, const ContactRecord &rec)
{
    ...
    auto ret = sys::Bus::SendUnicast(msg, ServiceDB::serviceName, serv, 5000);
    DBContactResponseMessage *contactResponse = reinterpret_cast<DBContactResponseMessage *>(ret.second.get());
    if ((ret.first == sys::ReturnCodes::Success) && (contactResponse->retCode == true)) { return true; }
    else
    {
        return false;
    }
...
uint32_t DBServiceAPI::ContactGetCount(sys::Service *serv, bool favourites)
{
    ...
    auto ret = sys::Bus::SendUnicast(msg, ServiceDB::serviceName, serv, 5000);
    DBContactResponseMessage *contactResponse = reinterpret_cast<DBContactResponseMessage *>(ret.second.get());
    if ((ret.first == sys::ReturnCodes::Success) && (contactResponse->retCode == true)) { return contactResponse->count; }
    else
    {
        return false;
    }
...
bool DBServiceAPI::AlarmAdd(sys::Service *serv, const AlarmsRecord &rec)
{
    ...
    if ((ret.first == sys::ReturnCodes::Success) && (alarmResponse->retCode == true)) { return true; }
    else
    {
        return false;
    }
...
uint32_t DBServiceAPI::AlarmGetCount(sys::Service *serv)
{
    ...
    auto ret = sys::Bus::SendUnicast(msg, ServiceDB::serviceName, serv, 5000);
    DBAlarmResponseMessage *alarmResponse = reinterpret_cast<DBAlarmResponseMessage *>(ret.second.get());
    if ((ret.first == sys::ReturnCodes::Success) && (alarmResponse->retCode == true)) { return alarmResponse->count; }
    else
    {
        return false;
    }
...
std::unique_ptr<std::vector<AlarmsRecord>> DBServiceAPI::AlarmGetLimitOffset(sys::Service *serv, uint32_t offset, uint32_t limit)
{
    ...
    auto ret = sys::Bus::SendUnicast(msg, ServiceDB::serviceName, serv, 5000);
    DBAlarmResponseMessage *alarmResponse = reinterpret_cast<DBAlarmResponseMessage *>(ret.second.get());
    if ((ret.first == sys::ReturnCodes::Success) && (alarmResponse->retCode == true)) { return std::move(alarmResponse->records); }
    else
    {
...
AlarmsRecord DBServiceAPI::AlarmGetNext(sys::Service *serv, time_t time)
{
    ...
    auto ret = sys::Bus::SendUnicast(msg, ServiceDB::serviceName, serv, 5000);
    DBAlarmResponseMessage *alarmResponse = reinterpret_cast<DBAlarmResponseMessage *>(ret.second.get());
    if ((ret.first == sys::ReturnCodes::Success) && (alarmResponse->retCode == true)) { return std::move((*alarmResponse->records)[0]); }
    else
    {
        return AlarmsRecord{};
    }
...
bool DBServiceAPI::NotesAdd(sys::Service *serv, const NotesRecord &rec)
{
    ...
    if ((ret.first == sys::ReturnCodes::Success) && (notesResponse->retCode == true)) { return true; }
    else
    {
        return false;
    }
...
uint32_t DBServiceAPI::NotesGetCount(sys::Service *serv)
{
    ...
    auto ret = sys::Bus::SendUnicast(msg, ServiceDB::serviceName, serv, 5000);
    DBNotesResponseMessage *notesResponse = reinterpret_cast<DBNotesResponseMessage *>(ret.second.get());
    if ((ret.first == sys::ReturnCodes::Success) && (notesResponse->retCode == true)) { return notesResponse->count; }
    else
    {
        return false;
    }
...
bool DBServiceAPI::CalllogAdd(sys::Service *serv, const CalllogRecord &rec)
{
    ...
    if ((ret.first == sys::ReturnCodes::Success) && (calllogResponse->retCode == true)) { return true; }
    else
    {
        return false;
    }
...
uint32_t DBServiceAPI::CalllogGetCount(sys::Service *serv)
{
    ...
    auto ret = sys::Bus::SendUnicast(msg, ServiceDB::serviceName, serv, 5000);
    DBCalllogResponseMessage *calllogResponse = reinterpret_cast<DBCalllogResponseMessage *>(ret.second.get());
    if ((ret.first == sys::ReturnCodes::Success) && (calllogResponse->retCode == true)) { return calllogResponse->count; }
    else
    {
        return false;
    }
