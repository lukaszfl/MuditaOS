...
namespace gui
{
    ...
    template <class T> class StateIcon : public ImageBoxWithText
    {
      public:
        ...
        StateIcon()   = delete;
        StateIcon(Item *parent,
                  T state,
                  const IconMap &data,
                  gui::ImageTypeSpecifier imgType = gui::ImageTypeSpecifier::None)
            : ImageBoxWithText(
                  parent, new gui::Image(data.at(state).first, imgType), utils::translate(data.at(state).second)),
              state(state), data(data), imgType(imgType)
        {
        ...
        {
            this->state = state;
            image->set(data.at(state).first, imgType);
      ...
      protected:
        T state;
        const IconMap data;
        gui::ImageTypeSpecifier imgType;
    };
} // namespace gui
