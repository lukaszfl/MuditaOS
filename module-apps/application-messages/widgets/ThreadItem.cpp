...
void ThreadItem::setThread( std::shared_ptr<ThreadRecord>& threadRecord ) {
	this->threadRecord = threadRecord;
	//set values of the labels
	title->setText( std::to_string( threadRecord->dbID));
	snippet->setText( threadRecord->snippet );
}
