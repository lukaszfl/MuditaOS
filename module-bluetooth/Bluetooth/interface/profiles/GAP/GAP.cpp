...
namespace bluetooth
{
    ...
    void GAP::setOwnerService(sys::Service *service) noexcept
    {
    ...
    {
        auto result = std::find_if(std::begin(devs), std::end(devs), [&addr](Devicei &device) {
    ...
    void GAP::sendDevices()
    {
        ...
        sys::Bus::SendUnicast(msg, "ApplicationSettings", ownerService);
        sys::Bus::SendUnicast(msg, "ApplicationSettingsNew", ownerService);
    }
    ...
    auto GAP::remoteNameToFetch() -> bool
    {
        ...
        auto index = std::distance(std::end(devices), result);
        return index != 0;
    }
