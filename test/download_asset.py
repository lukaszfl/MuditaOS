...
class Getter(object):
    '''Download latest ecoboot.bin image'''

    def __init__(self):
        self.host = 'https://api.github.com/repos'
        self.organization = 'mudita'
        self.repo = 'ecoboot'
        self.apitoken = None
        self.ghLogin = None
        self.getGitRoot()
    ...
    def genRestPrefix(self):
        ...
        self.restPrefix += '/'
        self.restPrefix += self.organization
        self.restPrefix += '/'
    ...
    def getApiToken(self, args=None):
        if args is not None and "token" in dir(args):
    ...
    def getGHLogin(self, args=None):
        if args is not None and "login" in dir(args):
    ...
    def getReleases(self, args):
        ...
        while count == itemsOnPage:
            ...
            if response.status_code != requests.codes.ok:
                log.error(f"download error: {response.status_code}")
    ...
    def listReleases(self, args):
        ...
        log.info(" tag | name | date | pre-release")
        log.info(tabulate(self.releases))
        ...
        if args.tag is None:
            ...
            for rel in self.releases:
                if rel['tag_name'] == args.tag:
                    ...
                    break
        if release is None:
            log.error("No release with tag:", args.tag)
        log.info("tag:", args.tag)
        log.debug("asset:", args.assetRepoName)
        log.debug("release:", release['tag_name'])
        assets = release['assets']

        for asset in assets:
            asset_name = asset['name']
            log.debug(f"=> asset name: {asset_name}")
            if asset_name == args.assetRepoName:
                self.downloadAsset(asset, args.assetOutName)
                if 'repository' in dir(args):
    ...
    def downloadAsset(self, asset, outName):
        self.createWorkdir()
        log.debug("name:", asset['name'])
        log.debug("url:", asset['url'])
        log.debug("size:", asset['size'])
        ...
        log.debug("downloading to:", destination_file)
        ...
        with open(destination_file, 'wb') as fd:
            for chunk in response.iter_content(chunk_size=1024):
                ...
                fd.write(chunk)
        if response.status_code != requests.codes.ok:
            log.error("download error:", response.status_code)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Download ecoboot")
